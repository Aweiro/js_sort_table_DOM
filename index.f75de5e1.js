...
title.forEach((element, index)=>{
    element.addEventListener("click", (e)=>{
        ...
        tbody.sort((a, b)=>{
            ...
            if (cleanNumber(aText) && cleanNumber(bText) && !isNaN(cleanNumber(aText)) && !isNaN(cleanNumber(bText))) return cleanNumber(aText) - cleanNumber(bText);
            return aText.localeCompare(bText);
        });
        tbodyOld.innerHTML = "";
        for (const key of tbody)tbodyOld.appendChild(key);
    });
});
